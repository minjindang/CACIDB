...
/// <summary>
/// RPOUT_Statics_01BL 的摘要描述
/// </summary>
public class RPOUT_Statics_01BL : ICommonBL, IQueryBL
{
    #region IQueryBL 成員
    ...
    private string getDefaultSql()
    {
        string sqlstr = "SELECT CAST(YEAR(GETDATE()) - 1911 AS nvarchar) + '/' + CAST(MONTH(GETDATE()) AS nvarchar) + '/' + CAST(DAY(GETDATE()) AS nvarchar) AS today, " + 
                        ...
                        "FROM SysCode AS b LEFT OUTER JOIN Consulting AS a ON a.Cnst_CntWay = b.Sys_CdCode AND YEAR(a.Cnst_CntDate) = @Cnst_CntDate " +
                        "WHERE (b.Sys_CdKind = 'C') AND (b.Sys_CdType = 'I') AND 1=1";        
        return sqlstr;
    }
    ...
    DataTable IQueryBL.QueryDataForList(DataTO to) 
    {
        ...
        cmd.CommandText += " GROUP BY a.Cnst_CntWay,b.Sys_CdText";
