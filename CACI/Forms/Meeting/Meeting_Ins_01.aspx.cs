...
public partial class Meeting_Ins_01 : IMMDInsertUI
{
    ...
    public override void SetDefault()
    {
        ...
        queryTo.setValue("Mety_CanAdd", "Y");
        ddl_Meeting_Class.DataSource = bf.getTableData("MeetingType", queryTo);
    ...
    private void Bind_ProjectDetail()
    {
        ...
        if (projectInfo != null & projectInfo.Rows.Count > 0)
        {
            ...
            if(this.lbl_Pj_Kind.Text == "A")
                this.lbl_Pj_Kind_ch.Text = "獎補助專案";
            else
                this.lbl_Pj_Kind_ch.Text = "輔導專案";
            if (ddl_Meeting_Class.SelectedValue == "AR")//獎補助審查會議不需要設定委員
            {
    ...
    protected void ddl_Meeting_Class_SelectedIndexChanged(object sender, EventArgs e)
    {
        Bind_Pj_Code();
        Bind_ProjectDetail();
    }

    protected void ddl_Pj_Code_SelectedIndexChanged(object sender, EventArgs e)
    {
        Bind_ProjectDetail();
    }
    ...
    protected void TabContainer1_ActiveTabChanged(object sender, EventArgs e)
    {
        ...
        if (TabContainer1.ActiveTab.HeaderText == "顧問委員")
        {
            if (dt != null && dt.Rows.Count > 0)
            {
                ddl_Meeting_Index.Items.Clear();
                ddl_Meeting_Index.DataSource = dt;
                ddl_Meeting_Index.DataTextField = "Meeting_Index";
                ddl_Meeting_Index.DataValueField = "Meeting_Index";
                ddl_Meeting_Index.DataBind();
                up_Tab2.Update();
            }
        }
        else if (TabContainer1.ActiveTab.HeaderText == "申請單位")
        {
            if (dt != null && dt.Rows.Count > 0)
            {
                ddl_Meeting_Index2.Items.Clear();
                ...
                if (ddl_Meeting_Class.SelectedValue == "AR")//獎補助審查會議不需要設定委員
                {
                    ddl_Committee.Items.Clear();
                    ddl_Committee.Items.Add(new ListItem("獎補助審查會議不需要設定委員", ""));
                    ddl_Committee.Enabled = false;
                }
                else
                {
                    ddl_Committee.Enabled = true;
                    ddl_Committee.Items.Clear();
                    ddl_Committee.DataSource = ICommonUI.GridView2DataTable(grv_Committee);
                    ddl_Committee.DataTextField = "Comm_Name";
                    ddl_Committee.DataValueField = "Comm_Code";
    ...
    protected void Committee_SelectAll(object sender, EventArgs e)
    {
        bool isChecked = ((CheckBox)(this.grv_SearchCommittee.HeaderRow.Cells[0].FindControl("cbHead"))).Checked;
        foreach (GridViewRow gvRow in grv_SearchCommittee.Rows)
        {
            ((CheckBox)(gvRow.Cells[0].FindControl("cbItem"))).Checked = isChecked;
        }
    }

    protected override DataTO PopulateDetailMarkedData(string DetailGridViewID)
    {
        ...
        switch (DetailGridViewID)
        {
            case "grv_Committee":
                ...
                {
                    to.setValue("Meeting_Index", ddl_Meeting_Index.SelectedValue);
                }
                break;
            case "grv_Company":
                if (!to.isColumnExist("Meeting_Index"))
    ...
    protected override void AfterHandleInsert()
    {
        base.AfterHandleInsert();
        btnDTL_UPDATE.Enabled = false;
    }
}