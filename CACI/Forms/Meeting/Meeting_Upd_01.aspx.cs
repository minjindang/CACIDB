...
public partial class Meeting_Upd_01 : IMMDUpdateUI
{
    ...
    public override DataTO PopulateDetailData(string DetailGridViewID)
    {
        ...
        switch (DetailGridViewID)
        {
            ...
            case "grv_Committee":
                ...
                string metyCode = string.Empty;
                DataTO queryTo = new DataTO();
                queryTo.setValue("Meeting_Code", this.lbl_Meeting_Code.Text);
                DataTable dtMeeting = new BaseFun().getTableData("Meeting", queryTo);
                if (dtMeeting != null && dtMeeting.Rows.Count > 0)
                    metyCode = dtMeeting.Rows[0]["Meeting_Class"].ToString();//會議類別
                if (metyCode == "CC")//顧問駐診會議
                    to.setValue("Comm_CoachWay", "001");
                else
                {
                    if (lbl_Pj_Kind.Text == "A")
                        to.setValue("Comm_CoachWay", "100");
                    else
                        to.setValue("Comm_CoachWay", "010");
                }
